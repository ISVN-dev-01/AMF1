#!/usr/bin/env python3
"""
PHASE 6.2: XGBoost Regression Model for Pole Prediction
Train XGBoost to predict qualifying times, then rank for pole prediction
"""
...
def train_xgboost_regressor(X_train, y_train, groups_train, X_val=None, y_val=None):
    """Train XGBoost regression model with GroupKFold cross-validation"""
    ...
    if X_val is not None and y_val is not None:
        ...
        # Reduce n_estimators for final training without validation
...
def plot_feature_importance(feature_names, importance_values, top_n=15):
    ...
    # Save plot
    plot_file = Path('reports/xgboost_feature_importance.png')
...
def run_xgboost_stage1():
    """Run complete XGBoost Stage-1 pole prediction"""
    
    print("=" * 80)
    print("PHASE 6.2: XGBOOST REGRESSION FOR POLE PREDICTION")
    print("=" * 80)
    ...
    model, cv_scores, feature_importance = train_xgboost_regressor(
    ...
    # Save trained model
    model_file = models_dir / 'xgboost_stage1_regressor.pkl'
    ...
    for split_name, split_results in pole_results.items():
        pred_file = reports_dir / f'xgboost_pole_predictions_{split_name}.csv'
    ...
    summary_file = reports_dir / 'xgboost_stage1_summary.csv'
    summary_df.to_csv(summary_file, index=False)
    print(f"   Summary: {summary_file}")
    
    # Final summary
    print(f"\n✅ XGBoost Stage-1 Complete!")
    ...
    try:
        ...
        if baseline_file.exists():
            ...
            xgb_train_acc = pole_results['train']['metrics']['top1_accuracy']
            
            improvement = xgb_train_acc - baseline_train_acc
            
            print(f"\n🎯 vs FP3 Baseline:")
            print(f"   Baseline Top-1: {baseline_train_acc:.3f}")
            print(f"   XGBoost Top-1:  {xgb_train_acc:.3f}")
...
if __name__ == "__main__":
    results = run_xgboost_stage1()
    
    # Save complete results
    results_file = Path('data/models/xgboost_stage1_results.pkl')
