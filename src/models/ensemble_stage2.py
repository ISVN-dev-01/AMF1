#!/usr/bin/env python3
"""
PHASE 7.2A: Ensemble Classifier for Race Winner Prediction
Fast ensemble approach: XGBoost + LightGBM + RandomForest → Logistic Regression meta-learner
"""
...
def train_base_models(X_train, y_train, groups_train):
    ...
    # 2. Gradient Boosting (fallback if XGBoost/LightGBM unavailable)
    if HAS_XGBOOST:
        ...
        try:
            ...
            print(f"   XGBoost failed: {e}, using sklearn GradientBoosting")
        ...
        print(f"   XGBoost not available, using sklearn GradientBoosting...")
        gb_model = GradientBoostingClassifier(
            n_estimators=100,
            max_depth=6,
            learning_rate=0.1,
    ...
    # 3. LightGBM or additional Gradient Boosting
    if HAS_LIGHTGBM:
        ...
        try:
            ...
            print(f"   LightGBM failed: {e}, using additional RandomForest")
            ...
            base_models['random_forest_2'] = rf2_model
    else:
        print(f"   LightGBM not available, using additional RandomForest...")
        rf2_model = RandomForestClassifier(
            n_estimators=150,
            max_depth=12,
            min_samples_split=3,
...
def generate_base_predictions(base_models, X):
    ...
    for model_name, model in base_models.items():
        if model_name == 'lightgbm':
            # Handle LightGBM prediction format
            pred_proba = model.predict(X)
            # LightGBM returns single probability, convert to 2-class format
            pred_proba_2d = np.column_stack([1 - pred_proba, pred_proba])
        else:
            # Standard sklearn prediction format
            pred_proba_2d = model.predict_proba(X)
        
        # Use probability of positive class (winner)
        base_predictions[model_name] = pred_proba_2d[:, 1]
...
def run_ensemble_classifier():
    ...
    print("=" * 80)
    print("PHASE 7.2A: ENSEMBLE CLASSIFIER FOR RACE WINNER PREDICTION")
    print("=" * 80)
